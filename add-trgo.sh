...
until [[ $user =~ ^[a-zA-Z0-9_]+$ && ${user_EXISTS} == '0' ]]; do
		...
		user_EXISTS=$(grep -w $user /etc/trojan-go-william/akun.conf | wc -l)
...
sed -i '/"'""$uuid""'"$/a\,"'""$user""'"' /etc/trojan-go-william/config.json
exp=`date -d "$masaaktif days" +"%Y-%m-%d"`
echo -e "### $user $exp" >> /etc/trojan-go-william/akun.conf
